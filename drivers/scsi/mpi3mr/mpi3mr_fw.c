...
int mpi3mr_blk_mq_poll(struct Scsi_Host *shost, unsigned int queue_num)
{
	...
	if (!atomic_add_unless(&mrioc->op_reply_qinfo[queue_num].busy_mq_poll,
	    1, 1))
		return 0;
	...
	atomic_dec(&mrioc->op_reply_qinfo[queue_num].busy_mq_poll);
	return num_entries;
}
 ...
 * are exepcted, schedule an IRE polling thread.
 *
...
static int mpi3mr_create_op_reply_q(struct mpi3mr_ioc *mrioc, u16 qidx)
{
	...
	atomic_set(&op_reply_q->busy_mq_poll, 0);
...
static int mpi3mr_print_pkg_ver(struct mpi3mr_ioc *mrioc)
{
	...
	void *data = NULL;
	dma_addr_t data_dma;
	struct mpi3_component_image_header *ci_header;
	u32 data_len = sizeof(*ci_header);
	u8 sgl_flags = MPI3MR_SGEFLAGS_SYSTEM_SIMPLE_END_OF_LIST;
	...
	ci_upload.image_offset = MPI3_IMAGE_HEADER_SIGNATURE0_OFFSET;
	ci_upload.segment_size = MPI3_IMAGE_HEADER_SIZE;
	...
	if ((mrioc->init_cmds.ioc_status & MPI3_IOCSTATUS_STATUS_MASK)
	    == MPI3_IOCSTATUS_SUCCESS) {
		ci_header = (struct mpi3_component_image_header *) data;
		ioc_info(mrioc, "firmware package version(%d.%d.%d.%d.%05d-%05d)\n",
		    ci_header->component_image_version.gen_major,
		    ci_header->component_image_version.gen_minor,
		    ci_header->component_image_version.phase_major,
		    ci_header->component_image_version.phase_minor,
		    ci_header->component_image_version.customer_id,
		    ci_header->component_image_version.build_num);
	}
	retval = 0;
out_unlock:
	mrioc->init_cmds.state = MPI3MR_CMD_NOTUSED;
...
void mpi3mr_memset_buffers(struct mpi3mr_ioc *mrioc)
{
	...
	for (i = 0; i < mrioc->num_queues; i++) {
		...
		atomic_set(&mrioc->op_reply_qinfo[i].busy_mq_poll, 0);
	...
	u32 reset_reason, u8 snapdump)
{
	...
	if (retval) {
		ioc_err(mrioc, "failed to issue soft reset to the controller\n");
		goto out;
	}
